...
def upload_file_with_retry(file_path, file_type):
    """آپلود فایل با تلاش مجدد"""
    ...
    for attempt in range(RETRY_COUNT):
        try:
            ...
            if resp_upload.status_code != 200:
                logging.warning(f"requestSendFile ناموفق (تلاش {attempt+1})")
            ...
            if upload_data.get("status") != "OK":
                logging.warning(f"requestSendFile خطا: {upload_data.get('status')}")
                if attempt < RETRY_COUNT - 1:
                    time.sleep(2)
                continue
            ...
            if resp_file.status_code != 200:
                logging.warning(f"آپلود فایل ناموفق (تلاش {attempt+1})")
                if attempt < RETRY_COUNT - 1:
                    time.sleep(2)
                continue
            ...
            if file_data.get("status") != "OK":
                logging.warning(f"آپلود خطا: {file_data.get('status')}")
                if attempt < RETRY_COUNT - 1:
                    time.sleep(2)
                continue
                
            return file_data["data"]["file_id"]
            
        except Exception as e:
            logging.warning(f"خطا در آپلود (تلاش {attempt+1}): {e}")
            if attempt < RETRY_COUNT - 1:
                time.sleep(2)
...
def send_to_rubika(content_type, file_path=None, caption=""):
    """ارسال به روبیکا با آپلود پایدار"""
    if not RUBIKA_TOKEN:
        return False
    ...
    try:
        if content_type == "text":
            ...
            # آپلود با تلاش مجدد
            file_id = upload_file_with_retry(file_path, file_type)
            if not file_id:
                logging.error(f"❌ آپلود {content_type} ناموفق بعد از {RETRY_COUNT} تلاش")
                return False
        ...
        logging.error(f"❌ خطا در send_to_rubika: {e}")
        return False
...
def scrape():
    print("=" * 55)
    print("       اسکرپ کانال خبری - NewsLine360 (نسخه پایدار)")
    print("=" * 55)
